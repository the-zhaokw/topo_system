...
def _check_proj_perm(user, perm_code):
    ...
    if user.check_permission(perm_code):
        return True
    # project:view / risk:view 权限额外允许项目成员访问
    if perm_code in ('project:view', 'risk:view'):
        try:
            from enhanced_app import ProjectMember
            member = _proj_db.session.query(ProjectMember).filter_by(user_id=user.id).first()
            if member:
                logger.info(f"用户 {user.username}(id={user.id}) 通过项目成员身份获得 {perm_code} 权限")
                return True
            else:
...
def update_project(project_id):
    ...
    # 延迟导入权限装饰器    current_user_id = get_jwt_identity()
    data = request.get_json()
...
def get_project_members(project_id):
    # 外层 require_project_permission('project:view') 已完成 JWT 验证和权限检查
    log_manager = get_log_manager_safe()
    db, Project, User, ProjectMember, ProjectStatus, create_audit_log, UserRole, Bug = get_db_and_models()
...
def delete_project(project_id):
    # 外层 require_project_permission('project:delete') 已完成 JWT 验证和权限检查
    log_manager = get_log_manager_safe()
    db, Project, User, ProjectMember, ProjectStatus, create_audit_log, UserRole, Bug = get_db_and_models()
