...
contracts_statistics_api = Api(contracts_statistics_bp)


class GlobalDeliveryDashboardResource(Resource):
    """全球项目交付看板 API"""
...
class RevenueForecastResource(Resource):
    """收入预测 API"""
    method_decorators = {'get': [jwt_required()]}
...
class RiskAnalysisDashboardResource(Resource):
    """风险分析仪表板 API"""
    method_decorators = {'get': [jwt_required()]}
    ...
    def get(self):
        ...
        return {
            ...
            'high_risk_contracts': high_risk_contracts[:20],  # 限制返回数量
...
class ContractTypeAnalysisResource(Resource):
    """合同类型分析 API"""
    method_decorators = {'get': [jwt_required()]}
    ...
    def get(self):
        ...
        查询参数：
        time_range: 时间范围（last_30_days/last_90_days/last_year/all）
        """
...
class RegionalPerformanceResource(Resource):
    """区域绩效分析 API"""
    method_decorators = {'get': [jwt_required()]}
...
class ExecutiveDashboardResource(Resource):
    """高管驾驶舱 API - 综合数据汇总"""
    method_decorators = {'get': [jwt_required()]}
...
# 注册路由
contracts_statistics_api.add_resource(GlobalDeliveryDashboardResource, '/global-delivery')
contracts_statistics_api.add_resource(RevenueForecastResource, '/revenue-forecast')
contracts_statistics_api.add_resource(RiskAnalysisDashboardResource, '/risk-analysis')
contracts_statistics_api.add_resource(ContractTypeAnalysisResource, '/type-analysis')
contracts_statistics_api.add_resource(RegionalPerformanceResource, '/regional-performance')
contracts_statistics_api.add_resource(ExecutiveDashboardResource, '/executive-dashboard')
